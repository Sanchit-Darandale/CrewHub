...
from models import Database, User, Worker, Authentication, Appointment, Review, Report, Bill
...
if cloudinary_url:
    # Set it in environment for the SDK to pick up
    os.environ["CLOUDINARY_URL"] = cloudinary_url
    # Also explicitly configure to be safe
    try:
...
def inject_admin_status():
    if session.get('admin'):
        ...
        return {'pending_worker_count': pending_count}
    return {'pending_worker_count': 0}
...
def admin_dashboard():
    ...
    banned_workers = len(list(filter(lambda w: w.get('is_banned'), worker_model.get_all_workers())))
    ...
    for report in reports_list:
        ...
        if worker:
            report['worker_name'] = worker.get('full_name')
            report['worker_email'] = worker.get('email')
...
@app.route('/admin/ban_worker/<worker_id>/<int:ban>')
@admin_required
def ban_worker(worker_id, ban):
    """Ban or unban a worker"""
    is_banned = True if ban == 1 else False
    worker_model.update_ban_status(worker_id, is_banned)
...
def report_worker(worker_id):
    ...
    report_model.create(worker_id, reporter_id, reason)
